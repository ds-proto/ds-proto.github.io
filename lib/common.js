...
function createMap() {
  // Create a choropleth
  d3.csv("data/aid_FY2014.csv", function(dataset) {
    clean_data = dataset.filter(function (record) {
      return record["dac_category_name"] == "Health and Population" && record["country_name"] != "" && record["country_code"] != "WLD" && record["country_code"] != "";
    });
    var health2014Values = clean_data.map(function(obj)
      { return obj["constant_amount"]; }
    );
    var minValue = Math.min.apply(null, health2014Values),
        maxValue = Math.max.apply(null, health2014Values),
        mapData = {};
    var paletteScale = d3.scale.linear()
          .domain([minValue, maxValue])
          .range(["#EFEFFF","#00254A"]); // dark dark blue
    clean_data.forEach(function(item) {
      // item is one record
      var countryCode = item["country_code"],
          expenditure = item["constant_amount"],
          countryName = item["country_name"];
      mapData[countryCode] = {
        name: countryName,
        healthSpending: expenditure,
    ...
    var heatmap = new Datamap({
      ...
      geographyConfig: {
          ...
          popupTemplate: function(geo, data) {
              ...
              if (!data) { return ; }
              // tooltip content
              console.log("amount is " + data["constant_amount"]);
              return ['<div class="hoverinfo">',
                  '<strong>', geo.properties.name, '</strong>',
                  '<br>Spending: <strong>', data["constant_amount"], '</strong>',
                  '</div>'].join('');
          }
      }
    }); //new Datamap
...
// heatmap uses lat/long data from https://opendata.socrata.com/dataset/Country-List-ISO-3166-Codes-Latitude-Longitude/mnkm-8ram
