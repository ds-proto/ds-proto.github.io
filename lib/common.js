...
function init(state) {
  createBubble(state.data,
               ...
               function (myChart, data, x, y, z) {
                 myChart.draw();
                 d3.select('#predictor-list')
                   .selectAll('li')
                   .data(state.predictors)
                   .enter()
                   .append('li')
                   .text(String)
                   .classed('selected', function (d) {
                     return d === state.predictor;
                   })
                   .on('click', function (d) {
                     appstate.predictor = d;
                     myChart.axes[0].measure = appstate.predictor;
                     // myChart.data = data.filter(function (record) {
                     //   return record[appstate.predictor] != "";
                     // });
                     myChart.draw(1000);
                   });

                 d3.select('#response-list')
                   .selectAll('li')
                   ...
                   .text(String)
                   .classed('selected', function (d) {
                     return d === state.response;
                   })
                   .on('click', function (d) {
                     appstate.response = d;
                     myChart.axes[1].measure = appstate.response;
                     myChart.data = data.filter(function (record) {
                       return record[appstate.response] != "";
                     });
                     myChart.draw(1000);
                   });
               });
